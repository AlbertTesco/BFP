...
double Arbitrary_triangle::Get_angel_bottom_left() {
    return (acos( ( -pow(this->right_side, 2) + pow(this->left_side, 2) + pow(this->bottom_side, 2) ) / ( 2 * this->left_side * bottom_side ) ) * 180.0 / 3.14);
}
double Arbitrary_triangle::Get_angel_bottom_right() {
    return (asin(this->Get_hight() / this -> right_side) * 180.0 /3.14);
}

double Arbitrary_triangle::Get_angel_top() {
    return ( 180.0 - ( this->Get_angel_bottom_right() + this -> Get_angel_bottom_left()) );
}
